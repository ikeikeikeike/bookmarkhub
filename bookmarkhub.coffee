...
class BH.Counter
  ...
  stumbleupon: (callback) ->
    @cachedRequest BH.URLs.stumbleupon(@url), dataType: 'json', (data) ->
      callback data.query
        .results.json.result?.views or 0
...
class BH.Linker
  ...
  twitter: (callback) ->
    ...
    @counter.twitter (count) ->
      callback
        count: count
        link: "https://twitter.com/search?q=#{me.schemelessUrl()}" if count

  facebook: (callback) ->
    @counter.facebook (count) ->
      callback
        count: count

  hatena: (callback) ->
    me = @
    @counter.hatena (count) ->
      callback
        count: count
        link: "http://b.hatena.ne.jp/entry/#{me.schemelessUrl()}" if count

  google: (callback) ->
    @counter.google (count) ->
      callback
        count: count

  pocket: (callback) ->
    @counter.pocket (count) ->
      callback
        count: count

  linkedin: (callback) ->
    me = @
    @counter.linkedin (count) ->
      callback
        count: count

  delicious: (callback) ->
    me = @
    @counter.delicious (count) ->
      callback
        count: count
        link: "https://previous.delicious.com/url/#{BH.md5Hex me.url}" if count

  pinterest: (callback) ->
    me = @
    @counter.pinterest (count) ->
      callback
        count: count

  stumbleupon: (callback) ->
    me = @
    @counter.stumbleupon (count) ->
      callback
        count: count
